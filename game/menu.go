...
func (ui *UIPage) Update() error {
	...
	for i := range ui.floatingElements {
		...
		if fe.x > screenWidth+50 {
			fe.x = -50
		}
		if fe.y < -50 {
			fe.y = screenHeight + 50
		}
		if fe.y > screenHeight+50 {
			fe.y = -50
		}
	}
...
func (ui *UIPage) Draw(screen *ebiten.Image) {
	...
	// Reuse pre-allocated temp screen for shake effect
	if ui.tempScreen == nil {
		ui.tempScreen = ebiten.NewImage(screenWidth, screenHeight)
	}
	tempScreen := ui.tempScreen
	tempScreen.Clear()
