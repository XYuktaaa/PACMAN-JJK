...
func (ui *UIPage) drawCharacterArea(screen *ebiten.Image) {
	// Character area dimensions and position (now for static character image or placeholder)
	charAreaX := float32(screenWidth - 300)
	charAreaY := float32(50)
	charAreaWidth := float32(250)
	charAreaHeight := float32(250)
	
	// Draw placeholder frame with modern styling (GIF is now in menu background)
	frameColor := color.RGBA{100, 150, 255, 80}
	vector.StrokeRect(screen, charAreaX, charAreaY, charAreaWidth, charAreaHeight, 3, frameColor, false)
	...
	// Placeholder text
	placeholderText := "STATIC"
	textX := int(charAreaX + charAreaWidth/2 - float32(len(placeholderText)*4))
	textY := int(charAreaY + charAreaHeight/2)
	ui.drawGlowText(screen, placeholderText, textX, textY, 
		color.RGBA{150, 150, 200, 150}, 1.0)
	
	subText := "CHARACTER AREA"
	subTextX := int(charAreaX + charAreaWidth/2 - float32(len(subText)*3))
	subTextY := textY + 20
	ui.drawGlowText(screen, subText, subTextX, subTextY, 
		color.RGBA{100, 100, 150, 100}, 0.8)
...
func (ui *UIPage) drawSleekMenu(screen *ebiten.Image) {
	...
	// Calculate menu panel dimensions
	panelHeight := float32(len(ui.menuOptions)*menuSpacing + 40)
	panelX := float32(menuX-40)
	panelY := float32(menuStartY-20)
	panelW := float32(menuWidth+80)
	
	// Draw GIF background for menu if available
	if len(ui.gifFrames) > 0 && ui.gifFrames[ui.frameIndex] != nil {
		currentFrame := ui.gifFrames[ui.frameIndex]
		...
		// Calculate scaling to fit menu panel (crop/scale to fill)
		scaleX := float64(panelW) / frameWidth
		scaleY := float64(panelHeight) / frameHeight
		
		// Use the larger scale to fill the menu area completely
		scale := math.Max(scaleX, scaleY)
		
		// Calculate position to center the scaled image within menu area
		scaledWidth := frameWidth * scale
		scaledHeight := frameHeight * scale
		offsetX := float64(panelX) + (float64(panelW)-scaledWidth)/2
		offsetY := float64(panelY) + (float64(panelHeight)-scaledHeight)/2
		
		// Create a clipping mask for the menu area
		op := &ebiten.DrawImageOptions{}
		op.GeoM.Scale(scale, scale)
		op.GeoM.Translate(offsetX, offsetY)
		
		// Make it semi-transparent so text is readable
		op.ColorM.Scale(1, 1, 1, .6) // 60% opacity
		...
		tempOp.ColorM.Scale(1, 1, 1, 0.4)
		tempImg.DrawImage(currentFrame, tempOp)
		
		// Draw the clipped GIF to the screen
		finalOp := &ebiten.DrawImageOptions{}
		finalOp.GeoM.Translate(float64(panelX), float64(panelY))
		screen.DrawImage(tempImg, finalOp)
	}
	
	// Draw menu background panel (semi-transparent overlay)
	panelColor := color.RGBA{15, 25, 45, 120} // More transparent to show GIF
	vector.DrawFilledRect(screen, panelX, panelY, panelW, panelHeight, panelColor, false)
	
	// Panel border
	borderColor := color.RGBA{100, 150, 255, 150}
	vector.StrokeRect(screen, panelX, panelY, panelW, panelHeight, 2, borderColor, false)
