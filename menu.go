...
func (ui *UIPage) Update(g *Game) error {
    if ui.startButtonPressed {
		g.showMenu = false
	}
	...
	if glowIntensity > 1.0 {
		...
		for _, offset := range offsets {
			for scale := 1; scale <= int(glowIntensity); scale++ {
				...
				text.Draw(screen, txt, bigfont, x+dx, y+dy+15, glowColor)
				text.Draw(screen, txt, bigfont, x+dx+1, y+dy+15, glowColor)
				text.Draw(screen, txt, bigfont, x+dx, y+dy+16, glowColor)
				text.Draw(screen, txt, bigfont, x+dx+1, y+dy+16, glowColor)
			}
		}
	}
